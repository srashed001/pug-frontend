...
function ActiveNav({game, joined}){
  ...
  >
    {my.username === game.createdBy.username && (<Button sx={{color: 'rgba(22, 26, 29)'}} 
      onClick={handleUpdateGame}
    >
      update game
    </Button>)}
    <Button sx={{color: 'rgba(102, 7, 8)'}}  onClick={handleCreateInvites}>invite players</Button>
    {my.username !== game.createdBy.username ? joined ? <Button sx={{color: 'rgba(102, 7, 8)'}}  onClick={handleLeaveGame}>leave game</Button> :  <Button sx={{color: 'rgba(102, 7, 8)'}}  onClick={handleJoinGame}>join game</Button> : null}
  </Paper>
