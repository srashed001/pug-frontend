...
function MessagesListNav({ thread, openDelete, handleOpenDelete }) {
  ...
  return (
    <Stack sx={{ position: 'fixed', top: '3.5rem',  zIndex: 20, width: '100%', backgroundColor: '#ffffff'}}>
      <Box sx={{ display: "inline-flex", boxShadow: 4, padding: 1, paddingTop: 2 }}>
        <Box>
          <Button
        ...
        <Box sx={{marginX: 2}}>
          {thread.party.length > 2 ? (
            <AvatarGroup max={3} spacing={20}>
              {party.map((user) => (
                  <Avatar sx={{backgroundColor: '#ffffff', boxShadow: 3}} src={user.profileImg} />
                )
              )}
            </AvatarGroup>
          ) : (
